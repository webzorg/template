...
module BaseHelper
  ...
  def alertifyjs_flash
    ...
    "<script>
      document.addEventListener('turbolinks:load', function() { alertify.dismissAll(); #{flash_messages.join} });
    </script>".html_safe
  end
  ...
  def index_actions_link_helper(action, item, data, permission_to_edit=true)
    ...
    when :show
      link_to action,
              ...
              class: "btn btn-info btn-sm btn-block #{'disabled' if !permission_to_edit}"
    when :edit
      link_to action,
              object_path_generator(action, item, data),
              class: "btn btn-warning btn-sm btn-block #{'disabled' if !permission_to_edit}"
    when :destroy
      link_to action,
              object_path_generator(action, item, data),
              method: :delete,
              data: { confirm: "Are you sure you want to delete #{data[:model]}?" },
              class: "btn btn-danger btn-sm btn-block #{'disabled' if !permission_to_edit}"
    end
  end
